function [solution] = relxopt(process_type,obj_type,para_X,sol1_database,sol2_database,M_site,Tm,DHf,P,P_lb,P_ub,T,T_lb,T_ub,molefrac_X_in,yield_lb,rho_guess,multistart_count)
%
% Associated Content:
...
% crystallization process optimization problem,  where the solvent PC-SAFT 
% pure component parameters and the process operating conditions are 
% optimized simultaneously (relaxed optimization problem).
%
% *************************************************************************
% To run 'relxopt', the user is required to have the function 'vert2con',
% which can be downloaded at:
% https://www.mathworks.com/matlabcentral/fileexchange/7895-vert2con-vertices-to-constraints
% *************************************************************************
%
...
%   composition, Stream S2 flow rate, (both the solvent and the antisolvent 
%   types are unknown);
...
% sol1_database should only include the known Solvent 1 candidate. See 
% Example 3.
%
%   Example 3: for a polar solvent,
%   sol1_database = [3.53	3.48	316.94	1822.33	0.009]
%
...
if process_type==21 || process_type==1 || process_type==3
    [A1,b1] = vert2con(sol1_database);
    
    cvN1.name='cvN1';
    cvN1.uels={1:1:length(A1)};
...
if process_type==21 || process_type==22
    [A2,b2] = vert2con(sol2_database);

    cvN2.name='cvN2';
    cvN2.uels={1:1:length(A2)};
...
for i=1:1:multistart_count
    ...
    if process_type==21
        pcsaft_para_sol=[0 0 0 0 0; 1+(3.5-1)*rand, 1+(3.5-1)*rand, 100+(350-100)*rand, 1000+(2500-1000)*rand, 0.1+(2-0.1)*rand; 1+(3.5-1)*rand, 1+(3.5-1)*rand, 100+(350-100)*rand, 1000+(2500-1000)*rand, 0.1+(2-0.1)*rand];
    elseif process_type==22
        pcsaft_para_sol=[0 0 0 0 0;sol1_database(1,:); 1+(3.5-1)*rand, 1+(3.5-1)*rand, 100+(350-100)*rand, 1000+(2500-1000)*rand, 0.1+(2-0.1)*rand];
    else
        pcsaft_para_sol=[0 0 0 0 0; 1+(3.5-1)*rand, 1+(3.5-1)*rand, 100+(350-100)*rand, 1000+(2500-50)*rand, 0.1+(2-0.1)*rand; 0 0 0 0 0];   
    end
    ...
    if process_type==21
        wgdx('relxopt_in', solute_para, assoc_site, melting_prop1, melting_prop2, pressure, temperature, cvN1, cvA1, cvb1, cvN2, cvA2, cvb2, para_sol, density_guess,obj);
        [out1, out2, out3, out4, out5, out6, out7, out8, out9, out10, out11, out12, out13, out14, out15]=gams('relxopt-type21');
    elseif process_type==22
        wgdx('relxopt_in', solute_para, assoc_site, melting_prop1, melting_prop2, pressure, temperature, mole_frac_in, cvN2, cvA2, cvb2, para_sol, density_guess,obj);
        [out1, out2, out3, out4, out5, out6, out7, out8, out9, out10, out11, out12, out13, out14, out15]=gams('relxopt-type22');
    elseif process_type==1
        wgdx('relxopt_in', solute_para, assoc_site, melting_prop1, melting_prop2, pressure, temperature, cvN1, cvA1, cvb1, para_sol, density_guess,obj);
        [out1, out2, out3, out4, out5, out6, out7, out8, out9, out10, out11, out12, out13, out14, out15]=gams('relxopt-type1');
    elseif process_type==3
        wgdx('relxopt_in', solute_para, assoc_site, melting_prop1, melting_prop2, pressure, pressure_lb, pressure_ub, temperature, temperature_lb, temperature_ub, y_lb, cvN1, cvA1, cvb1, para_sol, density_guess,obj);
        [out1, out2, out3, out4, out5, out6, out7, out8, out9, out10, out11, out12, out13, out14, out15]=gams('relxopt-type3');
    end
    ...
    if (out1.val==1 && out2.val==0)
       ...
       solution(j).m=out11.val;
       solution(j).sigma=out12.val;
       solution(j).epsilon=out13.val;
       solution(j).bi_eps=out14.val;
       solution(j).kappa=out15.val;
    end
