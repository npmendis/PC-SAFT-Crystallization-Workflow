...
*GAMS model for the relaxed optimization problem of Type 21
...
$load con_hull_A2=cvA2
$load con_hull_b1=cvb1
$load con_hull_b2=cvb2
...
m.l(sol_i)=pcsaft_para_sol_ini(sol_i,'1');
m.lo(sol_i)=1;
m.up(sol_i)=7;

sigma.l(sol_i)=pcsaft_para_sol_ini(sol_i,'2');
sigma.lo(sol_i)=1;
sigma.up(sol_i)=7;

epsilon.l(sol_i)=pcsaft_para_sol_ini(sol_i,'3');
epsilon.lo(sol_i)=50;
epsilon.up(sol_i)=5000;

bi_eps.l(sol_i)=pcsaft_para_sol_ini(sol_i,'4');
bi_eps.lo(sol_i)=500;
bi_eps.up(sol_i)=3500;

kappa.l(sol_i)=pcsaft_para_sol_ini(sol_i,'5');
kappa.lo(sol_i)=0.001;
kappa.up(sol_i)=2;
...
convex_hull_constraints2(conv2)
...
convex_hull_constraints2(conv2)..con_hull_A2(conv2,'1')*m('3')+con_hull_A2(conv2,'2')*sigma('3')+con_hull_A2(conv2,'3')*epsilon('3')+con_hull_A2(conv2,'4')*bi_eps('3')+con_hull_A2(conv2,'5')*kappa('3')=l=con_hull_b2(conv2);
