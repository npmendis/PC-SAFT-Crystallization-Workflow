$set matout "'relxopt_out.gdx', solver_status, infes, objval, T, P, rho_pure, rho, F, x, Q, m, sigma, epsilon, bi_eps, kappa"
...
*GAMS model for the relaxed optimization problem of Type 3
...
Set
   ...
   sol_i(i) solvents /2/
   state streams /1,3*5/
   sol_state(state) liquid streams /1,3,5/
   conv1 convex hull constraints (*)
...
$load P_lb=pressure_lb
$load P_ub=pressure_ub
$load T_init=temperature
$load T_lb=temperature_lb
$load T_ub=temperature_ub
$load yield_lb=y_lb
$load conv1=cvN1
$load con_hull_A1=cvA1
$load con_hull_b1=cvb1
...
Positive Variables
   ...
   x(i,state) composition (crystallizer)(molar fraction)
   Q heat duty in J
   ;
...
*caloric properties
   dddt(sol_i,sol_state)
   dzdt(n,sol_state)
   dghsdt(sol_i,sol_j,sol_state)
   ddijdt(sol_i,sol_j,sol_state)
   I1dt(sol_state)
   I2dt(sol_state)
   C1dt(sol_state)
   dahsdt(sol_state)
   ...
   daresdt(sol_state)
   daassocdt(sol_state)
   dassocdt(sol_i,sol_j,sol_state)
   dXAdt(sol_i,sol_state)
   hres(sol_state)
   h(sol_state)
;
...
*Compositions
x.l('1','1')=1*uniform(0.01,0.05);
x.l('2','1')=1-x.l('1','1');

x.l('1','3')=1*uniform(0.01,0.05);
x.l('2','3')=1-x.l('1','3');
...
m.l('2')=pcsaft_para_sol_ini('2','1');
*m.lo('2')=1;
*m.up('2')=7;

sigma.l('2')=pcsaft_para_sol_ini('2','2');
*sigma.lo('2')=1;
*sigma.up('2')=7;

epsilon.l('2')=pcsaft_para_sol_ini('2','3');
*epsilon.lo('2')=50;
*epsilon.up('2')=5000;

bi_eps.l('2')=pcsaft_para_sol_ini('2','4');
*bi_eps.lo('2')=50;
*bi_eps.up('2')=3500;

kappa.l('2')=pcsaft_para_sol_ini('2','5');
*kappa.lo('2')=0.001;
*kappa.up('2')=2;
...
pressure_constraint2

temperature_constraint1
temperature_constraint2

convex_hull_constraints1(conv1)
...
*caloric properties
CPig_define(sol_i,sol_state)
hig_define(sol_state)
dddt_define(sol_i,sol_state)
dzdt_define(n,sol_state)
dghsdt_define(sol_i,sol_j,sol_state)
I1dt_define(sol_state)
I2dt_define(sol_state)
C1dt_define(sol_state)
dahsdt_define(sol_state)
dahcdt_define(sol_state)
dadispdt_define(sol_state)
daresdt_define(sol_state)
ddijdt_define(sol_i,sol_j,sol_state)
dassocdt_define(sol_i,sol_j,sol_state)
dXAdt_define(sol_i,sol_state)
daassocdt_define(sol_state)
hres_define(sol_state)
h_define(sol_state)
;
...
mass_balance(i)..F('1')*x(i,'1')=e=F('3')*x(i,'3')+F('4')*x(i,'4')+F('5')*x(i,'5');

mole_frac_bound(state)..sum(i,x(i,state))=e=1;

energy_balance..Q=e=F('5')*h('5')+F('3')*h('3')-F('1')*h('1');
...
VL_equilibria(sol_i).. x(sol_i,'3')*exp(lnphi(sol_i,'3'))=e=x(sol_i,'5')*exp(lnphi(sol_i,'5'));

pressure_constraint1.. P('4')=e=P('3');
pressure_constraint2.. P('5')=e=P('3');

temperature_constraint1.. T('4')=e=T('3');
temperature_constraint2.. T('5')=e=T('3');

convex_hull_constraints1(conv1)..con_hull_A1(conv1,'1')*m('2')+con_hull_A1(conv1,'2')*sigma('2')+con_hull_A1(conv1,'3')*epsilon('2')+con_hull_A1(conv1,'4')*bi_eps('2')+con_hull_A1(conv1,'5')*kappa('2')=l=con_hull_b1(conv1);

obj_function1.. obj1=e=F('4');
obj_function2.. obj2=e=F('4')/(F('1')*x('2','1'));
obj_function3.. obj3=e=Q*1e-6/F('4');
...
CPig_define(sol_i,sol_state)..CPig(sol_i,sol_state)=e=-44.66+17.29*(m(sol_i)*epsilon(sol_i)/T(sol_state))+1.52*(m(sol_i)*sigma(sol_i)**3)-0.57*(m(sol_i)*sigma(sol_i)**3*epsilon(sol_i)/T(sol_state));

hig_define(sol_state)..hig(sol_state)=e=sum(sol_i,x(sol_i,sol_state)*CPig(sol_i,sol_state));

dddt_define(sol_i,sol_state)..dddt(sol_i,sol_state)=e=sigma(sol_i)*(3*epsilon(sol_i)/T(sol_state)/T(sol_state))*(-0.12*exp(-3*epsilon(sol_i)/T(sol_state)));

dzdt_define(n,sol_state)..dzdt(n,sol_state)=e=pi/6*rho(sol_state)*sum(sol_i,x(sol_i,sol_state)*m(sol_i)*np(n)*dddt(sol_i,sol_state)*power(d(sol_i,sol_state),np(n)-1));

dghsdt_define(sol_i,sol_j,sol_state)..dghsdt(sol_i,sol_j,sol_state)=e=dzdt('3',sol_state)/(1-z('3',sol_state))**2+ddijdt(sol_i,sol_j,sol_state)*3*z('2',sol_state)/(1-z('3',sol_state))**2+dij(sol_i,sol_j,sol_state)*(3*dzdt('2',sol_state)/(1-z('3',sol_state))**2+
                                          6*z('2',sol_state)*dzdt('3',sol_state)/(1-z('3',sol_state))**3)+2*dij(sol_i,sol_j,sol_state)*ddijdt(sol_i,sol_j,sol_state)*2*z('2',sol_state)**2/(1-z('3',sol_state))**3+dij(sol_i,sol_j,sol_state)*dij(sol_i,sol_j,sol_state)*(4*z('2',sol_state)*dzdt('2',sol_state)/(1-z('3',sol_state))**3+
                                          6*z('2',sol_state)**2*dzdt('3',sol_state)/(1-z('3',sol_state))**4);

I1dt_define(sol_state)..I1dt(sol_state)=e=sum(es_j,(a(es_j,sol_state)*es_jp(es_j)*dzdt('3',sol_state)*power(z('3',sol_state),es_jp(es_j)-1)));

I2dt_define(sol_state)..I2dt(sol_state)=e=sum(es_j,(b(es_j,sol_state)*es_jp(es_j)*dzdt('3',sol_state)*power(z('3',sol_state),es_jp(es_j)-1)));

C1dt_define(sol_state)..C1dt(sol_state)=e=dzdt('3',sol_state)*C2(sol_state);

dahsdt_define(sol_state)..dahsdt(sol_state)=e=1/z('0',sol_state)*(3*(dzdt('1',sol_state)*z('2',sol_state)+z('1',sol_state)*dzdt('2',sol_state))/(1-z('3',sol_state))+3*z('1',sol_state)*z('2',sol_state)*dzdt('3',sol_state)/(1-z('3',sol_state))**2+
                                   3*z('2',sol_state)**2*dzdt('2',sol_state)/z('3',sol_state)/(1-z('3',sol_state))**2+z('2',sol_state)**3*dzdt('3',sol_state)*(3*z('3',sol_state)-1)/z('3',sol_state)**2/(1-z('3',sol_state))**3+
                                  (3*z('2',sol_state)**2*dzdt('2',sol_state)*z('3',sol_state)-2*z('2',sol_state)**3*dzdt('3',sol_state))/z('3',sol_state)**3*log(1-z('3',sol_state))+(z('0',sol_state)-
                                   z('2',sol_state)**3/z('3',sol_state)**2)*dzdt('3',sol_state)/(1-z('3',sol_state)));

dahcdt_define(sol_state)..dahcdt(sol_state)=e=mm(sol_state)*dahsdt(sol_state)-sum(sol_i,x(sol_i,sol_state)*(m(sol_i)-1)/ghs(sol_i,sol_i,sol_state)*dghsdt(sol_i,sol_i,sol_state));

dadispdt_define(sol_state)..dadispdt(sol_state)=e=-2*pi*rho(sol_state)*(I1dt(sol_state)-I1(sol_state)/T(sol_state))*order1(sol_state)-pi*rho(sol_state)*mm(sol_state)*(C1dt(sol_state)*I2(sol_state)+C1(sol_state)*I2dt(sol_state)-2*C1(sol_state)*I2(sol_state)/T(sol_state))*order2(sol_state);

daresdt_define(sol_state)..daresdt(sol_state)=e=dahcdt(sol_state)+dadispdt(sol_state)+daassocdt(sol_state);

ddijdt_define(sol_i,sol_j,sol_state)..ddijdt(sol_i,sol_j,sol_state)=e=(d(sol_j,sol_state)-dij(sol_i,sol_j,sol_state))/(d(sol_i,sol_state)+d(sol_j,sol_state))*dddt(sol_i,sol_state)+(d(sol_i,sol_state)-dij(sol_i,sol_j,sol_state))/(d(sol_i,sol_state)+d(sol_j,sol_state))*dddt(sol_j,sol_state);

dassocdt_define(sol_i,sol_j,sol_state)..dassocdt(sol_i,sol_j,sol_state)=e=3/8*(d(sol_i,sol_state)+d(sol_j,sol_state))**2*(dddt(sol_i,sol_state)+dddt(sol_j,sol_state))*ghs(sol_i,sol_j,sol_state)*kappaij(sol_i,sol_j)*(exp(bi_epsij(sol_i,sol_j)/T(sol_state))-1)+(d(sol_i,sol_state)+d(sol_j,sol_state))**3/8*dghsdt(sol_i,sol_j,sol_state)*kappaij(sol_i,sol_j)*(exp(bi_epsij(sol_i,sol_j)/T(sol_state))-1)-(d(sol_i,sol_state)+d(sol_j,sol_state))**3/8*
                                                   ghs(sol_i,sol_j,sol_state)*kappaij(sol_i,sol_j)*bi_epsij(sol_i,sol_j)/T(sol_state)/T(sol_state)*exp(bi_epsij(sol_i,sol_j)/T(sol_state));

dXAdt_define(sol_i,sol_state)..dXAdt(sol_i,sol_state)=e=-(XA(sol_i,sol_state)*XA(sol_i,sol_state))*rho(sol_state)*(sum(sol_j,x(sol_j,sol_state)*M_site(sol_j)/2*dXAdt(sol_j,sol_state)*assoc(sol_i,sol_j,sol_state)
                     +x(sol_j,sol_state)*M_site(sol_j)/2*XA(sol_j,sol_state)*dassocdt(sol_i,sol_j,sol_state)));

daassocdt_define(sol_state)..daassocdt(sol_state)=e=sum(sol_i,x(sol_i,sol_state)*((M_site(sol_i)/XA(sol_i,sol_state)-M_site(sol_i)*0.5)*dXAdt(sol_i,sol_state)));

hres_define(sol_state)..hres(sol_state)=e=-T(sol_state)*daresdt(sol_state)+Ztot(sol_state)-1;

h_define(sol_state)..h(sol_state)=e=hres(sol_state)*R*T(sol_state)+hig(sol_state)*(T(sol_state)-298.15);
...
if (obj_type=1, solve relxopt using nlp maximizing obj1);
if (obj_type=2, solve relxopt using nlp maximizing obj2);
if (obj_type=3, solve relxopt using nlp minimizing obj3);

set stat /modelStat, solveStat/;
parameter returnStat(stat),infes, objval, solver_status;

returnStat('modelStat') = relxopt.modelstat;
returnStat('solveStat') = relxopt.solvestat;
infes = relxopt.suminfes;
objval = relxopt.objval;
solver_status = relxopt.solvestat;
